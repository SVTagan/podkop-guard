...
- Финальный публичный `install.sh` повторно прогнан на целевом Cudy TR3000 поверх работающей v0.2.2: `/bin/ash -n` и установка завершились с кодом `0`, установщик сообщил `LuCI integration: ENABLED (4 authenticated Custom Commands)`.
- До и после повторной установки количество именованных LuCI-секций осталось ровно `4`; дубликаты не появились.
